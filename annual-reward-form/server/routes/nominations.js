...
router.patch("/scores", async (req, res) => {
  ...
  try {
    ...
    for (const doc of nominations) {
      // Preserve existing scores and merge new ones
      const existingScores = doc.scores ? (doc.scores.toObject ? doc.scores.toObject() : doc.scores) : {};
      doc.scores = { ...existingScores, ...scores };

      // ⚠️ CRITICAL: Tell Mongoose that the nested 'scores' field has been modified
