...
const createArchive = (options = { zlib: { level: 5 } }) => {
  if (typeof ZipArchive === 'function') {
    return new ZipArchive(options);
  }
  throw new Error("ZipArchive export not found in archiver module.");
};
...
// NEW: Render score value inside the PDF box if it exists in baseProfile.scores
...
router.patch("/scores/:id", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Scores updated successfully", data: updatedNomination });
  } catch (err) {
...
router.patch("/scores", async (req, res) => {
  ...
  try {
    ...
    // Update scores for matching nominations
    await Nomination.updateMany(query, { $set: { scores: scores } });
...
// Helper to compile a PDFKit document into a Buffer asynchronously
...
router.get("/download-bulk-archive", async (req, res) => {
  try {
    ...
    // 2. Group document structures cleanly by employee + awardType
    ...
    // 3. Compile ALL PDF Buffers prior to opening stream headers
    const archiveFiles = [];
    for (const [key, matchingEntries] of recordsMap.entries()) {
      const baseProfile = matchingEntries[0];
      const pdfBuffer = await generateNomineePDFBuffer(matchingEntries, scoringGuides);
     
      const cleanName = (baseProfile.employeeName || 'Nominee').replace(/[/\\?%*:|"<>]/g, '_').replace(/\s+/g, '_');
      const cleanAward = (baseProfile.awardType || 'Award').replace(/[/\\?%*:|"<>]/g, '_').replace(/\s+/g, '_');
 
      archiveFiles.push({
        buffer: pdfBuffer,
        name: `${cleanName}_${cleanAward}_Report.pdf`
      });
    }
 
    // 4. Setup Response headers & stream archive safely
    ...
    archive.on('error', (err) => {
      ...
      if (!res.headersSent) {
        res.status(500).json({ error: `Archive Stream Error: ${err.message || String(err)}` });
      }
    });
    ...
    if (!res.headersSent) {
      res.status(500).json({ error: String(err?.message || err || "Failed to assemble unified package download asset zip archive.") });
    }
  }
});
